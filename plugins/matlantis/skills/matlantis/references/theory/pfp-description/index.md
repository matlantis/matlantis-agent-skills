...
#### 元素の対応状況および擬ポテンシャル ​
各元素の擬ポテンシャルの選択は、PBEモードとR2SCANモードで異なります。それぞれ以下の図に示す通りです。

##### R2SCAN
R2SCANモードにおける各元素の対応状況と擬ポテンシャルの選択は、以下の表に示す通りです。表の見方は次の通りです。

  * 擬ポテンシャル名が記載されている元素は、R2SCANモードが対応している元素です。セルの記載は採用しているVASPのPAW擬ポテンシャル名を表します。例えば`Li_sv`は`Li_sv`ポテンシャルを、接尾辞(`_sv`、`_pv`、`_d`、`_h`)のない`Co`は標準の`Co`ポテンシャルを使用していることを表します。
  * `-`と記載されている元素は、R2SCANモードでは対応していません。
  * 空欄は該当する元素が存在しない位置です。
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| La | Ce | Pr_h | Nd_h | Pm_h | Sm_h | Eu_h | Gd_h | Tb_h | Dy_h | Ho_h | Er_h | Tm_h | Yb_h | Lu |
...
##### PBE
PBEモードにおける各元素の対応状況と擬ポテンシャルの選択は、以下の表に示す通りです。表の見方は次の通りです。

  * 擬ポテンシャル名が記載されている元素は、PBEモードが対応している元素です。セルの記載は採用しているVASPのPAW擬ポテンシャル名を表します。例えば`Li_sv`は`Li_sv`ポテンシャルを、接尾辞(`_sv`、`_pv`、`_d`、`_3`)のない`Co`は標準の`Co`ポテンシャルを使用していることを表します。
  * `-`と記載されている元素は、PBEモードでは対応していません。
  * 空欄は該当する元素が存在しない位置です。
...
元素の対応状況については以下の通りです。
  ...
  * 各モードが再現する対象として主に注力している元素。 
    * 擬ポテンシャルの周期表にて、擬ポテンシャル名が記載されている元素
  * 各モードでは再現できないもの。
    * 擬ポテンシャルの周期表にて、`-`と記載されている元素
...
先述のように、PFPは複数のDFT計算条件による学習データを持っています。これに対応して、計算モードと呼ばれる機能を提供しています。計算モードを切り替えることによってどの学習データに合った挙動をするかを選べます。PFPの各バージョンで提供している計算モードおよびデフォルトのモードは以下の通りです。
  ...
  * v9.0.0 ~: `R2SCAN`、`PBE`、`PBE_U`、`WB97XD`。デフォルトは`R2SCAN`です。
...
`R2SCAN`をデフォルトのモードと位置づけています。`R2SCAN`は`PBE`モードと比べて、形成エネルギーや融点などの固体に関する物性、反応エネルギーなどの有機分子に関する物性に関して実験値に対する再現性の向上が見込まれます。`PBE`は従来から広く利用されてきた基本的なモードです。酸化物など、Hubbard補正が有効な系に対しては`PBE_U`を利用すると再現性が向上します。`WB97XD`は単分子を主眼としたモードです。分子の安定構造や反応の遷移状態を求める場合等には適していますが、液体等の分子間の相互作用が重要となる系の再現性は低いと考えられます。
...
## 2026.06
  * PFP v9.0.0に対応させました。
    * デフォルトの計算モードを`R2SCAN`に変更しました。
    * PBEモードとR2SCANモードで採用している擬ポテンシャルが異なるため、擬ポテンシャルの周期表をモードごとに別々の図に分割しました。
