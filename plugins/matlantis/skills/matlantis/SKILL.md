...
name: matlantis
description: >
  Guide for writing Python code using Matlantis, Matlantis Corp.'s cloud-based atomistic simulation platform
...
Matlantis は Matlantis(株) が提供するクラウドベースの原子シミュレーションプラットフォームです。
コアとなるのは PFP (Preferred Potential) で、グラフニューラルネットワークベースの汎用原子間ポテンシャルです。
DFT に近い精度を低コストで実現し、大規模（10,000原子以上）・長時間（数百万ステップ）の動力学計算が可能です。
...
- `python --version > /dev/null 2>&1` の終了コードが 1 の場合: Python はまだ使えない前提で処理を止め、次のように日本語で案内してください。
  - まず概要として「Python 環境が有効になっていません。いったん Claude を終了し、使いたい Python 環境を有効化してから Claude を再起動してください」と伝える。
  - そのうえで、引数まで含めたフルコマンドを案内する。現在 `python313` と `python311` の 2 つの環境が利用できます。
    - Python 3.13 環境を使いたい場合: `use_venv python313` をタイプしてから `claude` を起動してください。
    - Python 3.11 環境を使いたい場合: `use_venv python311` をタイプしてから `claude` を起動してください。
...
pfcc-extras             <- Utilities for modeling and analysis
```
...
計算モードの選択フローチャート、モード一覧（旧モード名・バージョン依存元素数）、対応元素、PFP の適用範囲外は
[pfp スキル](../pfp/SKILL.md) を参照してください。
...
| `pfp/` | PFPモデル・calc_mode選択・適用範囲 | モード選択時、PFPで何ができるかを知りたい場合 |
...
| `adsorption/` | 吸着 | 吸着構造構築/吸着エネルギー/サイト探索 |
